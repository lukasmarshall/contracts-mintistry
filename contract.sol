...
contract customCoin { 
  ...
  function transfer(address _to, uint256 _value) returns(bool success) {
    if (bal[msg.sender] < _value) return false;
  ...
  function transferFrom(address _from, address _to, uint256 _value) returns(bool success) {
    ...
    if (bal[_from] < _value) return false;
    bal[_from] -= _value;
    bal[_to] += _value;
  ...
  /*Return the bal of a given address - can already be used with balance() but complying with API standard here.*/
